...
func InitServer() *rpc.Server {
	...
	paymentRepo := repository.NewPaymentRepo(paymentDao)
	nativeApiService := InitNativeService()
	nativePayService := wechat.NewNativePayService(nativeApiService)
	paymentServer := service.NewPaymentServer(paymentRepo, nativePayService)
	client := InitRegistry()
	server := rpc.NewServer(paymentServer, client)
	return server
}
