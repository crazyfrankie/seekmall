...
func (d *SellerDao) QueryDraftExist(ctx context.Context, name string, uid int) (bool, error) {
	var draft ProductDraft
	err := d.db.WithContext(ctx).Model(&ProductDraft{}).Where("name = ? AND uid = ?", name, uid).Find(&draft).Error
	if err != nil {
		return false, err
	}
	if draft.Id == 0 {
