...
func NewServer(product *service.ProductServer, client *clientv3.Client) *Server {
	s := grpc.NewServer()
...
func registerService(cli *clientv3.Client, port string) error {
	em, err := endpoints.NewManager(cli, "service/product")
	if err != nil {
		return err
	}

	addr := "127.0.0.1" + port
	serviceKey := "/service/product/" + addr
